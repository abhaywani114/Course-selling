		...
		<div class="modal-body">
			<form method="post" id="new_course_form">
				...
				<div class="form-group">
				 ...
				 <div class="form-group">
					<input type="number" name="participant_price" class="form-control" placeholder="Participant Price (*All prices are in <?php echo e(env('CURRENCY_CODE')); ?>)" required>
				</div>

				 <div class="form-group">
					<input type="number" name="observer_price" class="form-control" placeholder="Observer Price (*All prices are in <?php echo e(env('CURRENCY_CODE')); ?>)" required>
				</div>
...
var courseTable = $('#course_table').DataTable({
	...
	"autoWidth": false,
	"ajax": {
		url:"<?php echo e(route('admin.courses.datatable_main')); ?>",
		type: "POST",
		"data": {
		}
	},
...
function save_course() {
   ...
   $.ajax({
        type: 'POST',
        url: "<?php echo e(route('admin.courses.new')); ?>",
...
function updateCourses(id) {
	$.post("<?php echo e(route('admin.courses.update')); ?>", {
		course_id:id
	}).done((res) => {
...
function mailCourses(id) {
	$.post("<?php echo e(route('admin.courses.mail_modal')); ?>", {
		course_id:id
	}).done((res) => {
	   if ($('.modal.in, .modal.show').length < 1) {
	        $('.modal-backdrop').remove();
...
function delete_course(id) {
	...
	if (if_confirm) {
		$.post("<?php echo e(route('admin.courses.delete')); ?>", {
			course_id: id
		}).done( (res) => {
...
function send_mail_ajax() {
   event.preventDefault();
   $.post("<?php echo e(route('admin.courses.send_mail')); ?>", $("#mail_course_form").serialize() ).done( (res) => {
   		if (res.success == true)
			messageModal(res.msg);
		else
			messageModal(res.error);
...
<?php echo $__env->make('layout.layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\Projects\project_700a\resources\views/admin/courses.blade.php ENDPATH**/ ?>