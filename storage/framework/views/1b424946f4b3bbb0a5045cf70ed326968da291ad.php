...
<script>
  var admin_table = $('#admin_table').DataTable({
      ...
      "autoWidth": false,
      "ajax": {
        url:"<?php echo e(route('admin.adminmanagement.datatable_main')); ?>",
        type: "POST",
        "data": {
        }
      },
  ...
  function save_admin() {
     ...
     $.post("<?php echo e(route('admin.adminmanagement.new')); ?>", $("#new_admin_form").serialize() ).done( (res) => {
        if (res.success == true)
 ...
 function updateAdmin(id) {
  $.post("<?php echo e(route('admin.adminmanagemen.update')); ?>", {
    admin_id:id
  }).done((res) => {
    $("#res").html(res);
...
function save_admin() {
   event.preventDefault();
   $.post("<?php echo e(route('admin.adminmanagement.new')); ?>", $("#update_admin_form").serialize() ).done( (res) => {
      if (res.success == true)
      messageModal(res.msg);
    else
      messageModal(res.error);
...
function delete_admin(id) {
  ...
  if (if_confirm) {
    $.post("<?php echo e(route('admin.adminmanagement.delete')); ?>", {
      course_id: id
    }).done( (res) => {
...
<?php echo $__env->make('layout\layout', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Server_Files\Network Venture\project_7001a\resources\views/admin/manage_admins.blade.php ENDPATH**/ ?>